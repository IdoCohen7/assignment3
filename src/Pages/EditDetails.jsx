...
export default function EditDetails() {
  ...
  useEffect(() => {
    ...
    if (loggedInUser) {
      setPreviewImage(loggedInUser.profilePicture || '');
    }
  }, []);
  ...
  const validationSchema = Yup.object({
    ...
    lastName: Yup.string()
      ...
      .required("שדה חובה"),
    username: Yup.string()
      .matches(/^[a-zA-Z0-9!@#$%^&*()_+=-]+$/, "שם המשתמש חייב להכיל רק אותיות באנגלית ותווים מיוחדים")
      .required("שדה חובה"),
  ...
  if (!user) {
    return <p className="text-center mt-5">לא נמצא משתמש מחובר. אנא התחבר מחדש.</p>;
  }
